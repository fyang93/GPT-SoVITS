...
if cnhubert_base_path is not None:
    tts_config.cnhubert_base_path = cnhubert_base_path
...
# 自动替换 engdict-hot.rep 中的一些词汇
with open('GPT_SoVITS/text/engdict-hot.rep', 'r', encoding='utf-8') as file:
    hotwords = []
    for line in file:
        line = line.strip()
...
def hot_replace(text):
    # 使用正则表达式将中文字符和英文字符之间添加分隔符
    text = re.sub(r'(?<=[^\u4e00-\u9fa5])(?=[\u4e00-\u9fa5])', '|', text)
    text = re.sub(r'(?<=[\u4e00-\u9fa5])(?=[^\u4e00-\u9fa5])', '|', text)
    for hotword in hotwords:
        # 使用忽略大小写的匹配
        pattern = re.compile(r'\b' + hotword + r'\b', re.IGNORECASE)
        text = pattern.sub(hotword.lower(), text)
    text = text.replace('|', '')
    return text
