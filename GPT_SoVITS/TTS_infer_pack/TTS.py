...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config]):
        ...
        self.cnhubert_model:CNHubert = None
    ...
    def _init_models(self,):
        ...
        self.init_cnhubert_weights(self.configs.cnhubert_base_path)
        # self.enable_half_precision(self.configs.is_half)
        
        
        
    def init_cnhubert_weights(self, base_path: str):
        print(f"Loading CNHuBERT weights from {base_path}")
        self.cnhubert_model = CNHubert(base_path)
        self.cnhubert_model=self.cnhubert_model.eval()
        self.cnhubert_model = self.cnhubert_model.to(self.configs.device)
    ...
    def enable_half_precision(self, enable: bool = True):
        ...
        if enable:
            ...
            if self.bert_model is not None:
                self.bert_model =self.bert_model.half()
            if self.cnhubert_model is not None:
                self.cnhubert_model = self.cnhubert_model.half()
        else:
            if self.t2s_model is not None:
            ...
            if self.cnhubert_model is not None:
                self.cnhubert_model = self.cnhubert_model.float()
    ...
    def set_device(self, device: torch.device):
        ...
        if self.cnhubert_model is not None:
            self.cnhubert_model = self.cnhubert_model.to(device)
